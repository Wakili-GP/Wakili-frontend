...
const IndexPage = () => {
  ...
  const handleLogout = async () => {
    try {
      await logout();
      toast.success("تم تسجيل الخروج بنجاح", { description: "نراك قريباً" });
    } catch {
      ...
      <nav className="border-b bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60 fixed top-0 w-full z-50 shadow-card">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            {/* Centered Nav Links */}
            <div className="hidden md:flex items-center justify-center space-x-1 space-x-reverse">
              {appNavTabs.map((tab) => (
                <button
                  key={tab.path}
                  onClick={() => handleNavClick(tab.path)}
                  className="cursor-pointer flex items-center py-2 px-3 rounded-lg transition-all duration-200 text-muted-foreground hover:text-foreground hover:bg-muted/50 font-medium"
                >
                  <tab.icon className="w-4 h-4 ml-1.5" />
                  <span>{tab.label}</span>
                </button>
              ))}
            </div>
                ...
                <>
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="ghost" className="cursor-pointer flex items-center space-x-2 space-x-reverse">
