...
export default function AiChatPage() {
    ...
    const askMutation = useMutation({
        mutationFn: (text: string) => chatbotService.ask({
            query: text,
            session_id: session_id as string,
            user_id: user?.id as string,
        }),
        onMutate: async (text) => {
            await queryClient.cancelQueries({ queryKey: ["chatMessages", session_id] });
            const previousMessages = queryClient.getQueryData<Message[]>(["chatMessages", session_id]);
            ...
            queryClient.setQueryData<Message[]>(["chatMessages", session_id], (old = []) => [...old, userMsg]);

            queryClient.setQueryData<ChatMeta[]>(["chatHistory", user?.id], (old = []) => {
                return old.map(c => 
                    c.session_id === session_id 
                        ? { 
                            ...
                            title: c.title === "محادثة جديدة" ? text.slice(0, 45) : c.title 
                          } 
                        : c
                );
            });
        ...
        onSuccess: (res) => {
            ...
            queryClient.setQueryData<Message[]>(["chatMessages", session_id], (old = []) => [...old, botMsg]);

            queryClient.setQueryData<ChatMeta[]>(["chatHistory", user?.id], (old = []) => {
                return old.map(c => 
                    c.session_id === session_id 
                        ? { ...c, last_message: res.answer.slice(0, 60), updated_at: new Date().toISOString() } 
                        : c
                );
            });
        },
        onError: (err, newTodo, context) => {
            queryClient.setQueryData(["chatMessages", session_id], context?.previousMessages);
    ...
    return (
        <div className="flex flex-col h-full bg-background" dir="rtl">
            ...
            <div className="h-14 border-b border-border flex items-center px-5 gap-3 bg-background/95 backdrop-blur-sm shrink-0">
                ...
                <span className="text-sm font-semibold text-foreground">المساعد القانوني</span>
            ...
            <div className="flex-1 overflow-y-auto">
                {isLoadingHistory ? (
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center px-6">
                        ...
                        <p className="text-sm text-muted-foreground">ابدأ بكتابة سؤالك القانوني</p>
                    </div>
                ) : (
                            ...
                            {messages.map((msg) => (
                                ...
                                >
                                    ...
                                    <div className="space-y-2 max-w-[78%] min-w-0">
                                        ...
                                        >
                                            <div className="whitespace-pre-line text-right">{msg.content}</div>
                                            <div
                                                className={cn(
                                                    "text-[10px] mt-1.5",
                                                    msg.sender === "user"
                ...
                <div className="max-w-2xl mx-auto">
                    ...
                    <div className="mt-2 flex items-start gap-1.5 text-[11px] text-muted-foreground px-1">
                        ...
                        <p className="text-right leading-relaxed">
                            هذا المساعد يقدم إرشاداً عاماً ولا يغني عن الاستشارة القانونية المتخصصة.
                        </p>
                    </div>
                </div>
            </div>
